CREATE DATABASE homework_2_hard;
...
SELECT mark, (SELECT COUNT(*) from auto) - COUNT(*) as other_car_count 
FROM auto
GROUP BY mark
ORDER BY mark;

-- 3. Вывести на экран людей которые живут в городах с населением больше 1000000, 
-- если людей с таким же именем нет в таблице MAN
SELECT * FROM man 
WHERE firstname NOT IN (SELECT firstname FROM man GROUP BY firstname HAVING COUNT(*) > 1) 
	AND citycode IN (SELECT citycode FROM city WHERE peoples > 1000000);
SELECT firstname FROM man GROUP BY firstname HAVING COUNT(*) > 1;
